...
- [ ] WHEN un producto tiene la etiqueta `iva-incluido` THE SYSTEM SHALL marcarlo con `ivaIncluido: true` y `sospechaIva: false`, y WHEN una pala frontal, pala cajón, pala niveladora, retroexcavadora o chipeadora sin la etiqueta no es un tractor vendido con la pala incluida THE SYSTEM SHALL marcarla con `sospechaIva: true`.
